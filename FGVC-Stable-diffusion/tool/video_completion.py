...
from spatial_inpaint import sd_inpaint
...
def video_completion(args):
    ...
    while(np.sum(mask_tofill) > 0):
        ...
        # sd 
        mask_tofill, video_comp = sd_inpaint(sd_pipe, mask_tofill, video_comp)
...
def video_completion_seamless(args):
    ...
    # Image inpainting model.
    #deepfill = DeepFillv1(pretrained_model=args.deepfill_model, image_shape=[imgH, imgW])
    print("Load Stable Diffusion Model for inpainting")
    sd_pipe = StableDiffusionInpaintPipeline.from_pretrained("runwayml/stable-diffusion-inpainting", torch_dtype=torch.float16).to("cuda")
    sd_pipe.enable_attention_slicing()
